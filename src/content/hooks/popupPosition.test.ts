...
describe("calculateHorizontalPosition", () => {
  ...
  it("centers popup on selection when space is available", () => {
    ...
    const x = calculateHorizontalPosition(rect, popupWidth, viewportWidth, margin);
    expect(x).toBe(290);
  });
  ...
  it("handles narrow viewport where popup fills available space", () => {
    ...
    const x = calculateHorizontalPosition(rect, popupWidth, narrowViewport, margin);
...
describe("calculateVerticalPosition", () => {
  ...
  it("places popup below selection when sufficient space", () => {
    ...
    const result = calculateVerticalPosition(rect, popupHeight, viewportHeight, margin);
  ...
  it("falls back to bottom when neither above nor below has full space", () => {
    ...
    const result = calculateVerticalPosition(rect, popupHeight, tinyViewport, margin);
  ...
  it("handles zero margin", () => {
    const rect = createRect({ top: 100, bottom: 120 });
    const result = calculateVerticalPosition(rect, popupHeight, viewportHeight, 0);
...
describe("calculatePopupPosition", () => {
  ...
  it("returns complete position object with defaults", () => {
    ...
    const result = calculatePopupPosition(rect, defaultOptions, defaultViewport);
  ...
  it("handles small viewport (mobile)", () => {
    ...
    expect(result.x).toBeLessThanOrEqual(mobileViewport.width - defaultOptions.popupWidth - defaultOptions.margin);
    expect(result.x).toBeGreaterThanOrEqual(defaultOptions.margin);
  });
  ...
  it("handles large viewport (desktop)", () => {
    ...
    const result = calculatePopupPosition(rect, defaultOptions, desktopViewport);
